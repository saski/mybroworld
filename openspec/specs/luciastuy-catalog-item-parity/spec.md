...
Define parity requirements for single catalog/portfolio item pages in the owned `luciastuy` theme so `/portfolio/{slug}` matches the live Glacier presentation (layout, metadata, gallery, navigation, typography) without commercial builder or gallery plugins.
