...
- [x] 0.7 Sync the 2026-07-23 production incidents: (a) `pinterest-for-woocommerce` triggered fatal memory exhaustion; fix attempted via `.user.ini` (ineffective on DonDominio). (b) `jetpack` triggered second fatal memory exhaustion; fix: renamed `jetpack` to `jetpack.deactivated` via FTP. DonDominio shared hosting overrides both `ini_set()` and `.user.ini` — `.htaccess` `php_value` also likely ineffective. Plugin deactivation is the only reliable memory relief. Updated plugin inventory (22→21 active plugins) and removal log.
