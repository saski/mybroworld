...
- [x] 0.7 Sync the 2026-07-23 production incidents: (a) `pinterest-for-woocommerce` triggered fatal memory exhaustion; fix attempted via `.user.ini` (ineffective on DonDominio). (b) `jetpack` triggered second fatal; deactivated via FTP. (c) Even after Jetpack deactivation, WordPress core and WooCommerce still exhausted memory — DonDominio has a hard 160MB PHP limit that ignores `ini_set()`, `.user.ini`, and `WP_MEMORY_LIMIT`. Fix: mass-deactivated 4 more heavy plugins (`google-site-kit`, `pinterest-for-woocommerce`, `reddit-for-woocommerce`, `google-listings-and-ads`). Total: 6 plugins deactivated. Site stable across 3 rounds. Updated plugin inventory (11 active plugins) and removal log.
- [x] 0.8 Sync the 2026-07-25 catalog system fixes: (a) Updated Apps Script Web App URL (`AKfycbwC0e_...`) — old deployment lost authorization. (b) Migrated OAuth identity from `mybrocorp@gmail.com` to `nacho.saski@gmail.com` across all infrastructure: Secret Manager `catalog-agent-config`, Secret Manager `catalog-agent-oauth-token`, Apps Script deployment execution identity. (c) Both queued catalog jobs (`catalog_20260725_172133_8273`, `catalog_20260513_110543_19b5`) completed successfully. Persisted infra account rule in `deployments.md`.
