...
4. Treat header cell notes as the live field-purpose contract for sheet work. Read them before filling broad ranges, and if a canonical header is missing a purpose note, add or repair the note instead of relying on hidden chat context.
5. If the task is to explain fields to end users, prefer header cell notes over comment threads because notes are persistent field help attached to the header itself.
6. Build inference and review rules from the sheet plus the project criteria reference before writing anything.
...
7. Classify each blank, suspect value, or inconsistent cell:
...
8. Inspect cell formatting before editing. If a normalized output column contains manual fill colors without a documented meaning or matching conditional rule, treat that formatting as a review issue and reset it to the neutral background.
...
9. When unresolved issues remain or the user asks for a guided pass, switch into manual review mode and build a review queue before editing ambiguous cells.
10. Batch edits into one coherent `batchUpdate`: write inferred or normalized values first, then color unresolved cells.
11. Report touched cells as `Filled`, `Normalized`, `Flagged`, and `Skipped`.
12. If the user introduced a new reusable review rule during the task, update this skill and `references/project-criteria.md` before finishing.
...
- Treat allowed values in a header note as value constraints, not as decision logic. For editorial or judgment fields such as `include_in_catalog`, do not default blanks from the fact that the note says `TRUE` or `FALSE`.
- Treat explicit destroyed or recycled wording in the title or notes as `status_normalized = archived` when no other status is present.
...
- When importing legacy year tabs into the consolidated catalog schema, first determine whether the source tab has canonical headers. If it does not, treat every non-empty source row, including row 1, as artwork data and map by the evidenced positional layout for that year.
- For legacy year-tab image imports, match images only from the corresponding Drive year folder and only when the filename/title or row-prefix evidence is deterministic. If the match is missing or ambiguous, leave image fields blank and add a `catalog_blocker` instead of guessing.
...
- For PDF catalog image review, prefer the matching `_CAT01` image. When the match is uniquely supported by a similar filename, use it and report the inference; otherwise fall back to the client-provided `image_main` and report the fallback. Do not choose arbitrarily between multiple similar `_CAT01` candidates. Omit only artworks with no usable image and report their source rows.
