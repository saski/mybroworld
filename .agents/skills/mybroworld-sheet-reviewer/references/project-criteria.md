...
- For PDF catalog image selection, prefer the matching `_CAT01` image in the configured Drive folders. Use a unique normalized title match first, then a uniquely best similar filename match. If there is no safe `_CAT01` match, render the client-provided `image_main` and show the fallback as a warning in the completed job result.
- Do not choose arbitrarily between multiple similar `_CAT01` candidates. If no usable image can be resolved, omit that artwork, continue the catalog when at least one artwork remains renderable, and warn with the exact sheet header and original spreadsheet row numbers. Fail only when no selected artwork can be rendered.
- Example: `image_main fell back to image_main because no unique _CAT01 image was found: 2025 rows 25, 26`; `image_main is missing and the artwork was omitted: 2026 rows 12, 19`.
