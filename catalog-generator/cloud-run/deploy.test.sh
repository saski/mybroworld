...
assert_contains "$DRY_RUN_OUTPUT" "--async" "dry-run should submit Cloud Build asynchronously"
assert_contains "$DRY_RUN_OUTPUT" "--suppress-logs" "dry-run should suppress Cloud Build log streaming"
assert_contains "$DRY_RUN_OUTPUT" "gcloud builds describe" "dry-run should poll Cloud Build status without streaming logs"
