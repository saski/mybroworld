...
7. Run one real queued `lucia-mybrocorp` catalog PDF render from WordPress and confirm `result_file_url` is written back to `catalog_jobs`.
...
- Customer-operated catalog generation is not yet fully verified; Cloud Run and monitoring are live, the production Apps Script Web App is updated for on-demand worker startup, Cloud Run IAM allows the Nacho-owned Apps Script to invoke the worker, and a direct Apps Script-triggered Cloud Run PDF job completed. The final proof still requires queueing/reviewing a catalog from the customer's mybro WordPress account and then pausing the legacy worker scheduler.
